  ...
  Class("IrcWrapper", {
    ...
    methods : {
      initialize : function (args) {
        ...
        this.bindings['353'] = this.bindings['353'] || [];
        ...
        this.bindings.join.push({
          callback : function (h) {
            if (!(h.location in this.channels)) {
              this._addChannel(new m.Channel({
                name : h.location
              }));
            }
        ...
        this.bindings['353'].push({
          callback : function (h) {
            // Empty since persons are initialized in _parseE, but the event
            // listener still has to be here.
          }
        });
      ...
      _parseE : function (raw, e) {
        ...
        if (raw === "353") {
          h.channel = this.getChannel(h.e.params[2]);
          h.people = [];
          var nicks = h.e.params[3].split(/\s+/);
          for (var i = 0; i < nicks.length; i++) {
            var nick = nicks[i];
            var person = this._personCreate({
              nick : nick
            });
            h.channel.addPerson(person);
      ...
      _personCreate : function (values) {
        values.nick = values.nick.replace(/^[^A-Z]+/ig, "");
